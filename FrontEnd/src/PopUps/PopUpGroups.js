...
export const GroupsPopUp = ({ prevInfo, setActive, mode, id }) => {
  ...
  useEffect(() => {
    fetch("https://warm-badlands-92671.herokuapp.com/api/v1/users", {
      method: "GET",
      /*body: JSON.stringify({
            title:,
            }),*/
      ...
      .then((data) => {
        for (let i = 0; i < data.data.length; i++) {
          var Estudiante = {
            name: data.data[i].first_name + " " + data.data[i].first_last_name,
            id: data.data[i]._id,
          };
  ...
  const Materias = [];
  useEffect(() => {
    fetch("https://warm-badlands-92671.herokuapp.com/api/v1/courses", {
      method: "GET",
      /*body: JSON.stringify({
           title:,
           }),*/
  ...
  const UpdateGroup = async () => {
    ...
    await axios
      .patch(url + "groups/" + objectID, data, config)
      .then((response) => {
        console.log(response.data);
      })
      .catch((error) => {
  ...
  return (
    <PopUpForm
      Header={mode === "edit" ? "Editar grupo" : "Alta de grupo"}
      activate={setActive}
      action={mode === "edit" ? UpdateGroup : CreateGroup}
    ...
    >
      <div className="row mb-4">
        <SelectionBoxObject
          name={"group_teacher"}
          headName="Maestro"
          placeholder="seleciona Maestro"
          list={Maestross}
          prevValue={prevInfo ? form.group_teacher : null}
          Handler={CreateNewGroup}
        />
        <SelectionBoxObject
          name={"course"}
          headName="Curso"
          placeholder="seleciona Curso"
          list={Cursoss}
          prevValue={prevInfo ? form.course : null}
          Handler={CreateNewGroup}
        />
        <SelectionBoxObject
          name={"semester"}
          ...
          list={Semestres}
          prevValue={prevInfo ? form.semester : null}
          Handler={CreateNewGroup}
        />
      </div>
      <div className="row mb-4">
        <SelectionBoxSingle
          ...
          list={YearsList(10)}
          prevValue={prevInfo ? form.year : null}
          Handler={CreateNewGroup}
        />
      </div>
      <div className="row mb-4">
        <SearchObjectSelection
          name={"group_members"}
          headName="Seleccionar Alumnos"
          list={Userss}
          prevValue={prevInfo ? form.group_members : null}
          onclickHandler={CreateArrayMembers}
        />
      </div>
    </PopUpForm>
  );
