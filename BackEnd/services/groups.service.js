...
class GroupsService {
  ...
  update_Faker(id, changes) {
    ...
    if (index === -1) throw new boom.notFound('Group not found: ' + id);
  ...
  getAll_Faker(size) {
    ...
    if (!groups) throw boom.notFound('Collection doesn´t exists');
  ...
  getById_Faker(id) {
    ...
    if (!Group) throw new boom.notFound('Group not found: ' + id);
    return Group;
  }
  //#endregion
}
