...
class sectionService {
  ...
  Fakerupdate(id, changes) {
    ...
    if (index === -1) throw new boom.notFound('Section not found: ' + id);
  ...
  FakergetAll(size) {
    ...
    if (!sections) throw boom.notFound('Collection doesn´t exists');
  ...
  FakergetById(id) {
    //const nId = parseInt(id);
    const sections = this.sections.find(
      (item) => item && item.sectionId === id
    );
    if (!sections) throw new boom.notFound('Section not found: ' + id);
    return sections;
  }
}
//#endregion
