...
class CoursesService {
  ...
  update_Faker(id, changes) {
    ...
    if (index === -1) throw new boom.notFound('Course not found: ' + id);
  ...
  getAll_Faker(size) {
    ...
    if (!courses) throw boom.notFound('Collection doesn´t exists');
    else if (courses.length <= 0)
  ...
  getById_Faker(id) {
    ...
    if (!Course) throw new boom.notFound('Course not found: ' + id);
    return Course;
  }
  //#endregion
}
